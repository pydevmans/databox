...
def test_login(client):
    res = client.post(
        "/login", data=dict(username="user", password="IncorrectPassword21!")
    )
...
def test_userdatabases(logged_user_client):
    ...
    res2 = logged_user_client.put(
        "/users/user/databases", data=dict(database="newname")
    )
...
def test_user_database(logged_user_client):
    ...
    shutil.copy(
        "database/usernames/user/backup.txt", "database/usernames/user/sample.txt"
    )
    res2 = logged_user_client.put(
        "/users/user/databases/sample", data=dict(database="helloworld")
    )
...
def test_interacdatabase(logged_user_client):
    ...
    assert (
        b"2616 Cathedral Bluffs Drive street, Corona, Guam, U5N 7J3 Canada" in res1.data
    )
...
def test_userdatabases_loggedout(client):
    ...
    assert (
        b"You either supplied the wrong credentials (e.g. a bad password)" in res1.data
    )
...
def test_user_database_loggedout(client):
    ...
    assert (
        b"You either supplied the wrong credentials (e.g. a bad password)" in res2.data
    )
    res3 = client.delete("/users/user/databases/test1")
    assert (
        b"You either supplied the wrong credentials (e.g. a bad password)" in res3.data
    )
